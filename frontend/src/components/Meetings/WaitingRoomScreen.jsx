...
import { requestAdmission } from "../../store/meetingSlice";
import { formatMeetingDateTime, getCurrentUser } from "./meetingUtils";
...
const WaitingRoomScreen = ({ meeting, basePath = "/meetings" }) => {
  ...
  const admittedUsers = useSelector((state) => state.meetings.admittedUsers[meeting.id] || []);
  ...
  }, [meeting.id, currentUser.id, currentUser.name, currentUser.role, dispatch]);
    ...
    <div className="meeting-waiting-room">
      <div className="meeting-waiting-card">
        ...
        <div className="meeting-waiting-info">
          <h3>{meeting.title}</h3>
          <p>
            <strong>Scheduled:</strong> {formatMeetingDateTime(meeting.dateTime)}
          </p>
          <p>
            <strong>Requested at:</strong> {new Date().toLocaleTimeString()}
          </p>
        </div>
        ...
        <button className="meeting-btn meeting-btn-danger meeting-waiting-leave" onClick={handleLeave}>
          <LogOut size={16} />
          Leave Waiting Room
        </button>
      </div>
